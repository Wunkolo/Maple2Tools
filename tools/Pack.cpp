...
bool PackFolder(
	const fs::path& TargetFolder, Maple2::Identifier PackVersion
);
...
int main(int argc, char* argv[])
{
	...
	for( std::size_t i = 1; i < static_cast<std::size_t>(argc); ++i )
	{
		...
		const std::string CurPackVersion = CurArg.substr(0, Sep);
		if( CurPackVersion.size() != 4 )
		{
			std::printf(
				"Invalid PackStream identifier: \'%s\'", CurPackVersion.c_str()
			);
		}
		...
		{
			std::printf(
				"Unknown PackStream version: \'%s\'", CurPackVersion.c_str()
			);
			continue;
		}
		}
...
bool MakePackFile(
	...
	std::ofstream& HeaderFile, std::ofstream& DataFile
)
{
	...
	for(
		...
		: fs::recursive_directory_iterator(
			TargetFolder, fs::directory_options::follow_directory_symlink
		)
	)
	{
		const auto& CurPath = CurFile.path();
		if( !fs::is_regular_file(CurPath)
			|| CurPath.extension() == ".m2h" || CurPath.extension() == ".m2d"
		)
		{
			// Skip non-files
		...
		const fs::path RelativePath = fs::path(
			CurPath.string().substr(TargetFolder.string().size() + 1)
		);

		// Create Encoded data
		std::ifstream CurFileStream(CurPath, std::ios::binary);
		std::string Encoded;
		...
		std::tie(
			...
			CurFileStream, PackTraits::IV_LUT, PackTraits::Key_LUT, true
		);
		CurFileStream.close();
	...
	std::tie(
		...
		FATString,
		PackTraits::IV_LUT, PackTraits::Key_LUT,
		true
	);
...
)
{
	...
	HeaderFile.open(
		TargetFile.replace_extension(".m2h"), std::ios::binary | std::ios::trunc
	);
	std::printf(
	...
	DataFile.open(
		TargetFile.replace_extension(".m2d"), std::ios::binary | std::ios::trunc
	);
	std::printf("Creating data file: %s\n", TargetFile.string().c_str());
	if( !DataFile.good() )
	{
		std::printf("Error creating file: %s\n", TargetFile.string().c_str());
		return false;
	...
	case Maple2::Identifier::MS2F:
	{
		return MakePackFile<Maple2::PackTraits::MS2F>(
			TargetFolder, HeaderFile, DataFile
		);
	}
	case Maple2::Identifier::NS2F:
	{
